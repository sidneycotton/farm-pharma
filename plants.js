...
function renderLog(genome, stage) {
  ...
  // Stage 0: freshly felled, full bark, no cut face detail yet (just laid there)
  const debarkPatchStage = 1;
  const splitStage = 2;
  const seasonStage = 3;
  const weatheredStage = 4;

  const debarked = stage >= debarkPatchStage;
  const split = stage >= splitStage;
  const seasoned = stage >= seasonStage;
  const weathered = stage >= weatheredStage;
  ...
  if (split) {
    const crackDepth = faceRy * 1.6 + (logH - faceRy) * Math.min((stageT - splitStage / (genome.stageCount - 1 || 1)) * 2 + 0.3, 1);
