...
function generateGenome(seed, type) {
  ...
  if (type === 'vine') {
    ...
    return Object.assign(base, {
      ...
      wallHeight: rng.range(220, 340),
      wallWidth: rng.range(90, 130),
      wallStyle,
...
// ---------- VINE renderer (climbing a tall wall that rises as the plant grows) ----------
function renderWall(genome, stage) {
  ...
  // whole scene reads as "being built" rather than a static prop. The wall
  // is tall and narrow (like the side of a building) so the vine has real
  // vertical surface to climb rather than sprawling sideways.
  const growth = (stage + 1) / genome.stageCount;
  const maxWallHeight = Math.min(genome.wallHeight, 400); // keep within the 500-tall canvas
  const minHeight = 16;
  const wallHeightNow = minHeight + (maxWallHeight - minHeight) * growth;
  const wallWidthNow = genome.wallWidth; // width is fixed — only height grows
...
function renderVine(genome, stage) {
  ...
  let x = 200 + rng.range(-8, 8);
  let y = 471;
  let angle = 0; // straight up (polar() treats 0deg as "up")
  let svg = wallSvg;
  let path = `M ${x.toFixed(1)} ${y.toFixed(1)} `;
  const decorations = [];
  const wallHalfWidth = genome.wallWidth / 2 - 8; // keep the climbing vine on the wall face

  for (let i = 0; i < activeSegments; i++) {
    const climbing = y > wallTop + 10; // still on the wall face
    const sway = genome.direction * genome.waviness * 0.25 * Math.sin(i * genome.curlTightness * 4) + rng.range(-4, 4);
    // While climbing, stay biased strongly upward with a gentle side-to-side
    // sway (like a real vine hugging a wall); once past the top of the wall,
    // let it curl outward more freely.
    if (climbing) {
      angle = sway * 0.5;
      // pull back toward center if the sway has drifted off the wall's edge
      if (x - 200 > wallHalfWidth) angle -= 25;
      if (x - 200 < -wallHalfWidth) angle += 25;
    } else {
      angle = angle + sway;
    }
